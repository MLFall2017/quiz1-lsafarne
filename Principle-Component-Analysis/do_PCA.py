...
def main():
    ...
    dataForAnalysis = pd.read_csv('SCLC_study_output_filtered.csv')
    dataForAnalysis=dataForAnalysis.values
    useCorr = False
    myPCAResults = d_PCA(x=dataForAnalysis, corr_logic=useCorr)
    cov_pca=np.cov(myPCAResults['loadings'],rowvar=False)
    print(myPCAResults["PC_variance"].shape)
    print("pca {}".format(myPCAResults["PC_variance"]))
    ...
    plt.show()
    print(myPCAResults['scores'])
    pcaCov=np.cov(myPCAResults['scores'][:,0],myPCAResults['scores'][:,1])
    print("pcaCov for pc1 and pca2 is{}".format(pcaCov[:,0]))
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
    ...
    dataReconstructed = np.matmul(myPCAResults['scores'], myPCAResults['loadings'].T)
    print(len(myPCAResults['loadings'][:,0]))
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
